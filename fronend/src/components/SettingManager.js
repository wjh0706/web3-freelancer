...
function SettingManager({ info }) {
    ...
    axios({
      ...
      .then((res) => {
        ...
        if (res.data.project.multi_view_fileId) {
          ...
          axios({
            method: "get",
            url: "http://localhost:3001/api/files/" + res.data.project.multi_view_fileId,
          })
            .then((res) => {
          ...
          axios({
            method: "get",
            url: "http://localhost:3001/api/files/download/" + res.data.project.multi_view_fileId,
          })
            .then((res) => {
              if (firstLoad) {
  ...
  const handleUpdateConfig = () => {
    ...
    const config_yaml = new File([yamlData], info.id + "_config.yaml", {
  ...
  const handleUpload = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        axios({
          method: "get",
          url: "http://localhost:3001/api/files/download/" + res.data.file.id,
        }).then((res) => {
          const data = yaml.load(res.data);
          setEpoch(data.train.epoch);
          setNumWorkers(data.train.num_workers);
  ...
  const handleDownload = (id, fileName) => {
    axios({
      method: "get",
      url: "http://localhost:3001/api/files/download/" + id,
      responseType: "blob",
    }).then((response) => {
              ...
              <Stack spacing={2}>
                <div>
                  <TextField
                    ...
                    value={epoch}
                    onChange={(e) => {
                      if (e.target.value) setEpoch(parseInt(e.target.value));
                      else setEpoch(0);
                    }}
                  />
                  <TextField
                  ...
                  <TextField
                    ...
                    onChange={(e) => {
                      if (e.target.value)
                        setBatchTrain(parseInt(e.target.value));
                      else setBatchTrain(0);
                    }}
                  />
                </div>
                <div>
                  <TextField
                    ...
                    onChange={(e) => {
                      if (e.target.value)
                        setNumTrainFrame(parseInt(e.target.value));
                      else setNumTrainFrame(0);
                    }}
                  />
                  <TextField
                    label="Ratio"
                    ...
                    value={ratio}
                    onChange={(e) => {
                      if (e.target.value) setRatio(parseInt(e.target.value));
                      else setRatio(0);
                    }}
                  />
                  <TextField
                    ...
                    onChange={(e) => {
                      if (e.target.value)
                        setBatchTest(parseInt(e.target.value));
                      else setBatchTest(0);
                    }}
                  />
                </div>
          ...
          {isUpload && (
            <CardContent>
              <Typography variant="body2" color="text.secondary">
                {"Project id is " +
                  info.id +
                  ". You may use it as the data root in your configuration file."}
              </Typography>
              <Typography variant="body2" color="text.secondary">
              ...
              <Stack spacing={1}>
                ...
                {error && (
                  <Alert severity="error">
                    You must have train.batch_size, train.epoch,
                    train.num_workers, num_train_frame ,ratio, test.batch_size
                    defined in the file.
                  </Alert>
                )}
              </Stack>
